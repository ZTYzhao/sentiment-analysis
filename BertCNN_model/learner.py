...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'E:\pythonProject\Confusion-all\Bert_model\output/' + self.cur_date.replace(":",'') + '_checkpoint.pt')
...
class Trainer(object):
    ...
    def fit(self, num_epochs, args, device='cpu'):
        """
    ...
    def predict(self, device='cuda:0', pbar=None):
        ...
        preds_dict = {
            'y_true': np.zeros([current_size,]),#用train-sentiment.txt时参数为【current_size,2】
            'y_pred': np.zeros([current_size,])
        }
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar(self.val_data_loader, parent=pbar, leave=(pbar is not None))):
                ...
                preds_dict['y_true'][current_index: current_index + num_rows] = targets#//用train-sentiment.txt时原语句为preds_dict['y_true'][current_index: current_index + num_rows, :]= targets
                preds_dict['y_pred'][current_index: current_index + num_rows] = y_pred#//用train-sentiment.txt时原语句为preds_dict['y_true'][current_index: current_index + num_rows, :]
                index_dict += num_rows
...
class EvaluateOnTest(object):
    ...
    def predict(self, device='cpu', pbar=None):
        ...
        preds_dict = {
            'y_true': np.zeros([current_size,]),#用test-sentiment.txt时参数为【current_size,2】
            'y_pred': np.zeros([current_size,])
        }
        start_time = time.time()
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar(self.test_data_loader, parent=pbar, leave=(pbar is not None))):
                _, num_rows, y_pred, targets,last_hidden_state,pooler_output = self.model(batch, device)
                current_index = index_dict
                preds_dict['y_true'][current_index: current_index + num_rows] = targets#//用train-sentiment.txt时原语句为preds_dict['y_true'][current_index: current_index + num_rows, :]= targets
