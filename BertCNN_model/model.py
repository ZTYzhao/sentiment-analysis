...
class BertEncoder(nn.Module):
    def __init__(self, lang='English'):
        ...
        if lang == 'English':
            self.bert = BertModel.from_pretrained(BERT_PATH,from_tf=True)#'bert-base-uncased'  BERT_PATH,from_tf=True
            self.token = BertTokenizer.from_pretrained(BERT_PATH)
...
class SpanEmo(nn.Module):
    ...
    def forward(self, batch, device):
        ...
        if self.joint_loss == 'joint':
            ...
            logits = logits.squeeze(-1)#这行当使用post.csv时需要添加，使用train-sentiment.txt时可删除
        ...
        return loss, num_rows, y_pred, targets.cpu().numpy(),last_hidden_state, pooler_output
    ...
    def corr_loss(y_hat, y_true, reduction='mean'):
        ...
        :return: loss
        """
        loss = torch.zeros(y_true.size(0)).cpu()
